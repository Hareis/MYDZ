(function ($) {
    jQuery.fn.imageMaps = function (options) {
        ...
        var GlobalObj = null;
        var ItemList = [];
        var GoodsStyle = { Left: 10, Top: 10, Width: 775, Height: 240 };
        var PId = $("input[name=PId]").val();
        var UId = $("input[name=UId]").val();
        var LImg = '../../Images/inv.jpg';
        var Single = 'false';
        var container = this;
        ...
        var PrintContentList = [
            ...
            { id: 6, name: "销售属性", width: 100, content: ["柠檬黄 XL", "柠檬黄 XL", "柠檬黄 XL"] },
            { id: 7, name: "数量", width: 60, content: ["1", "1", "1"] },
            { id: 8, name: "单价", width: 80, content: ["168.00", "168.00", "168.00"] },
            { id: 9, name: "优惠", width: 80, content: ["10.00", "10.00", "10.00"] },
            { id: 10, name: "小计", width: 80, content: ["158.00", "158.00", "158.00"] }
        ];
        ...
        function InitData() {
            $.post("/OrderManagement/PrintContent.html", {}, function (data) {
                ...
                InnerHtml.push('    </ul>');
                InnerHtml.push('    <ul class="toolbar_bd" style="height:' + 620 + 'px;">'); //parseInt(options.Size.height - 44)
        ...
        function InitList() {
            $.post("/OrderManagement/PrintDetail.html", { Id: PId },
                function (data) {
                    for (var i = 0; i < data.List.length; i++) {
        ...
        function CreateRect(obj, data) {
            ...
            if (parseInt(ids) == 0) {
                GoodsStyle.Left = left;
                GoodsStyle.Top = top;
                GoodsStyle.Width = width;
                GoodsStyle.Height = height;
                CreateGoodsList(item, fid, font, bold, align);
                return;
            }
        ...
        function CreateGoodsList(item, fid, font, bold, align) {
            var Html = CreateProductsTable(item.split(','), fid, font, bold, align);
            position_conrainer.append('<div class="map-position" style="padding:10px 0px;left:' + GoodsStyle.Left + 'px;top:' + GoodsStyle.Top + 'px;width:' + GoodsStyle.Width + 'px;height:' + GoodsStyle.Height + 'px;"><div class="map-position-bg"></div><span class="link-number-text">' + Html.join('') + '</span><span class="edit" title="设置" data-id="0" data-fid="1" data-font="12" data-bold="false" data-item="' + item + '" data-param="1" data-align="' + align + '">+</span><span class="delete ineffective" title="删除">X</span><span class="resize"></span></div>');
            bind_map_event();
            define_css();
        }

        function CreateProductsTable(IdList, fid, font, bold, align) {
            var Html = [];
            var Content = [];
            var family = $("select[name=font] option[value=" + fid + "]").text();

            Html.push('<table cellpadding="0" cellspacing="0" border="0" width="100%" border="0">');
            Html.push(' <tr>');
            for (var i = 0; i < PrintContentList.length; i++) {
                var tst = $.grep(IdList, function (n, j) { return n == PrintContentList[i].id; });
                if (tst.length <= 0) { continue; }

                var style = "font-family:'" + family + "';font-size:" + font + "px;font-weight:" + (eval(bold) ? "700" : "normal") + ";text-align:" + (align == 1 ? "left" : align == 2 ? "center" : align == 3 ? "right" : "left") + ";" + (PrintContentList[i].width <= 0 ? "" : "width:" + PrintContentList[i].width + "px;'") + "";
                Html.push("<th style=" + style + ">" + PrintContentList[i].name + "</th>");
                if (Content.length <= 0) {
                    for (var j = 0; j < PrintContentList[i].content.length; j++) {
                        Content.push([]);
                    }
                }

                for (var j = 0; j < PrintContentList[i].content.length; j++) {
                    Content[j].push({ id: PrintContentList[i].id, content: PrintContentList[i].content[j] });
                }
            }
            Html.push(' </tr>');
            for (var i = 0; i < Content.length; i++) {
                ...
                for (var j = 0; j < Content[i].length; j++) {
                    var content = Content[i][j].id == 2 ? '<img src="' + Content[i][j].content + '" />' : Content[i][j].content;
                    var style = Content[i][j].id == 3 ? ' class="tleft"' : '';
                    Html.push('<td' + style + '>' + content + '</td>');
                }
                Html.push('</tr>');
            }
        ...
        function bind_map_event() {
            ...
            $('.position-conrainer .map-position .edit').unbind('click').click(function () {
                ...
                if (id == 0) {
                    ...
                    var html = [];
                    for (var i = 0; i < PrintContentList.length; i++) {
                        var tst = $.grep(tmpList, function (n, j) { return n == PrintContentList[i].id; });
                        var ischk = tst.length > 0;
                        html.push('<label style="padding:5px 5px 5px 0px;" class="checkbox ' + (ischk ? "checked" : "") + '"><span><input value="' + PrintContentList[i].id + '" type="checkbox"' + (ischk ? "checked" : "") + ' hidefocus="true"></span><em>' + PrintContentList[i].name + '</em></label>');
                    }
                    tr_items.eq(1).show().find("td:last").empty().append(html.join(''));
                    txt = "商品详情";
                }
        ...
        function SaveConfig(state) {
            ...
            if (state) {
                ...
                if (id == 0) {
                    var html = CreateProductsTable(IdList);
            ...
            function () {
                ...
                if (Data.Status) {
                    $.post("/OrderManagement/Template.html", Data,
                        function (data) {
                            if (!data.Status) { alert("保存数据失败，请与管理员联系。"); }
                        }
                    ).done(
                        function () {
        ...
        function verify() {
            ...
            Data = { Status: true, PlaneId: PId, UserId: UId, LogisticsId: 0, Image: LImg, Width: lwidth.val().Trim(), Height: lheight.val().Trim(), IsSingle: Single };
